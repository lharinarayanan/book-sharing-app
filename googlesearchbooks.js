          ...
          autocomplete: function(id){
               ...
               data('ui-autocomplete')._renderItem = function(ul, item){
                   ...
                   i++;
                    let info = $('<div></div>').
     addClass('book-info').
   append('<div class="book-title" id="btitle'+i+'"'+ (typeof item.title === 'undefined' ? 'hidden' : '') + '>' + item.title + '</div>').
   append('<div class="book-subtitle" id="bsubtitle'+i+'"' + (typeof item.subtitle === 'undefined' ? 'hidden' : '') + '>' + item.subtitle + '</div>').
   append('<div class="book-authors"id="bauthors'+i+'"' + (typeof item.authors === 'undefined' ? 'hidden' : '') + '>' + item.authors + '</div>').
   append('<div class="book-description"id="btitle'+i+'"' + (typeof item.description === 'undefined' ? 'hidden' : '') + '>' + item.description + '</div>').
   append('<div class="book-image"id="bimage'+i+'"' + (typeof item.image === 'undefined' ? 'hidden' : '') + '>' + item.image + '</div>').   
   append('<div class="book-description"' + (typeof item.canonicalVolumeLink === 'undefined' ? 'hidden' : '') + '>' + item.canonicalVolumeLink + '</div>').
   append('<div style="width:100px; height:30px; background:#FF0000; color:#FFF;"  onclick="saveAuthorRecords('+i+');" >Save</div>'); 
                    return $('<li></li>').
...
$(document).ready(function(){
 ...
 function saveAuthorRecords(name) {
          alert("Your details are accepted successfully");
          var str1 = document.getElementById('btitle'+name).innerHTML;
          console.log(str1);
          var str2 = document.getElementById('bauthors'+name).innerHTML;
          var str3 = document.getElementById('bimage'+name).innerHTML;
          //alert(str3); 
          console.log(str3);        
          window.location.href = "booksave.php?title=" + str1 + "&author=" + str2 + "&image="+str3;
}
